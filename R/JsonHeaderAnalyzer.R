...
#' @examples
#' \dontrun{
#' # Internal function; see tests for usage
#' }
JsonHeaderAnalyzer <- function(JsonReturn, Selectedheader, use_field_names_in_headers) {
  ...
  if (length(CleanedHeaders) == 1) {
    CleanedHeaders <- CleanedHeaders[[1]]
  }
  ...
  if (Selectedheader == "title") {
    ...
    if (identical(names(headers), c("name", "type", "decimalChar"))) {
      ...
      headers2 <- headers[, ParameterName := data.table::fifelse(
        test = (!(type %in% c("string", "number")) & name != "date"),
        no = name,
        yes = paste0(name, ".", tolower(title))
      )][ParameterName == "instrument", ParameterName := "Instrument"][ParameterName == "date", ParameterName := "Date"]
