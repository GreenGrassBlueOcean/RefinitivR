...
StreamManager <- R6::R6Class("StreamManager",
  public = list(
    ...
    initialize = function(base_url = NULL, port = NULL, stream_type = "pricing") {
      ...
      # Reconnection state
      private$.user_initiated_close <- FALSE
      private$.reconnecting <- FALSE
      private$.reconnect_attempt <- 0L
      private$.reconnect_callbacks <- list(
        on_reconnecting = list(),
        on_reconnected = list()
      )
    },
    ...
    connect = function(force = FALSE, debug = FALSE) {
      ...
      # Reset user-initiated flag so future accidental closes trigger reconnect
      private$.user_initiated_close <- FALSE
    ...
    #' @description Disconnect from WebSocket (user-initiated)
    disconnect = function() {
      private$.user_initiated_close <- TRUE
      private$.reconnecting <- FALSE
      private$.reconnect_attempt <- 0L

      if (!is.null(private$.ws)) {
        # Neutralize handlers before closing to prevent post-close callbacks
        tryCatch(
          {
            private$.ws$onError(function(event) NULL)
            private$.ws$onClose(function(event) NULL)
            private$.ws$onMessage(function(event) NULL)
            private$.ws$onOpen(function(event) NULL)
            private$.ws$close()
          },
          error = function(e) {
      ...
      private$.connection_state <- "disconnected"
      private$.logged_in <- FALSE
      # User-initiated: clear everything
      private$.streams <- list()
      private$.message_handlers <- list()
    },

    #' @description Register a reconnection lifecycle callback
    #' @param event "on_reconnecting" or "on_reconnected"
    #' @param callback Function to call
    register_reconnect_callback = function(event, callback) {
      if (!event %in% c("on_reconnecting", "on_reconnected")) {
        stop("event must be 'on_reconnecting' or 'on_reconnected'")
      }
      if (!is.function(callback)) {
        stop("callback must be a function")
      }
      private$.reconnect_callbacks[[event]] <- c(
        private$.reconnect_callbacks[[event]],
        list(callback)
      )
    },
    ...
    #' @return Integer vector of stream IDs (one per instrument in the universe).
    #'   For a single-instrument definition this is length 1.
    subscribe = function(stream_def) {
      ...
      # Allocate stream IDs — one per instrument
      n_instruments <- length(stream_def$get_universe())
      ...
      first_id <- private$.stream_id_counter
      stream_ids <- seq.int(first_id, first_id + n_instruments - 1L)
      # Advance the counter past all allocated IDs
      private$.stream_id_counter <- first_id + n_instruments - 1L

      # Store stream definition under each allocated ID
      for (sid in stream_ids) {
        private$.streams[[as.character(sid)]] <- stream_def
      }

      debug <- getOption("refinitiv_streaming_debug", FALSE)

      # For pricing streams, wait for login
      if (private$.stream_type == "pricing") {
        ...
        if (!private$.logged_in) {
          if (debug) message("[DEBUG] Login not complete, queueing subscription for stream IDs ", paste(stream_ids, collapse = ", "))
          for (sid in stream_ids) {
            private$.pending_subscriptions[[as.character(sid)]] <- stream_def
          }
          return(stream_ids)
        }
      }
      ...
      private$.send_subscription_requests(stream_def, first_id, debug)
      return(stream_ids)
    },
  ...
  private = list(
    ...
    # Reconnection state (H4)
    .user_initiated_close = FALSE,
    .reconnecting = FALSE,
    .reconnect_attempt = 0L,
    .reconnect_callbacks = list(),

    # Send subscription requests for a stream definition.
    # `to_request()` returns either a single JSON string or a named list of
    # JSON strings (one per instrument).  This helper handles both.
    .send_subscription_requests = function(stream_def, first_id, debug = FALSE) {
      requests <- stream_def$to_request(streaming = TRUE, stream_id = first_id)

      send_one <- function(req, label) {
        if (debug) {
          message("[DEBUG] Sending subscription ", label, ":")
          message("[DEBUG] ", req)
        }
        if (!is.null(private$.ws)) {
          private$.ws$send(req)
          ...
          message("[DEBUG] WARNING: WebSocket is NULL, cannot send subscription")
        }
      }

      if (is.character(requests) && length(requests) == 1L && is.null(names(requests))) {
        # Single instrument — single JSON string
        send_one(requests, paste0("stream ", first_id))
      } else {
        # Multiple instruments — named list of JSON strings
        for (i in seq_along(requests)) {
          send_one(requests[[i]], paste0("stream ", first_id + i - 1L, " (", names(requests)[i], ")"))
        }
      }
    ...
    # --- Reconnection state machine (H4) ---

    .attempt_reconnect = function(debug = FALSE) {
      if (private$.user_initiated_close) {
        if (debug) message("[DEBUG] Reconnect aborted: user-initiated close")
        return()
      }

      max_retries <- getOption("refinitiv_streaming_max_reconnect", 10L)
      if (private$.reconnect_attempt >= max_retries) {
        if (debug) message("[DEBUG] Reconnect exhausted after ", max_retries, " attempts")
        warning("WebSocket reconnection failed after ", max_retries, " attempts")
        private$.reconnecting <- FALSE
        private$.reconnect_attempt <- 0L
        return()
      }
      ...
      private$.reconnect_attempt <- private$.reconnect_attempt + 1L

      # Delay sequence: [5, 10, 15, 60, 60, ...] with +/- 15% jitter
      delay_seq <- c(5, 10, 15, 60)
      base_delay <- delay_seq[min(private$.reconnect_attempt, length(delay_seq))]
      jitter_factor <- 1 + stats::runif(1, -0.15, 0.15)
      delay <- base_delay * jitter_factor

      if (debug) {
        message(
          "[DEBUG] Scheduling reconnect attempt ", private$.reconnect_attempt,
          "/", max_retries, " in ", round(delay, 1), "s"
        )
      }

      # Fire on_reconnecting callbacks
      private$.fire_reconnect_callbacks("on_reconnecting", private$.reconnect_attempt)

      if (!requireNamespace("later", quietly = TRUE)) {
        warning("'later' package required for auto-reconnection")
        private$.reconnecting <- FALSE
        return()
      }

      # Capture self reference for the later callback
      self_ref <- self

      later::later(function() {
        priv <- self_ref$.__enclos_env__$private
        # Guard: user may have disconnected while we were waiting
        if (priv$.user_initiated_close || !priv$.reconnecting) {
          if (debug) message("[DEBUG] Reconnect cancelled during delay")
          priv$.reconnecting <- FALSE
          return()
        }

        if (debug) message("[DEBUG] Reconnect attempt ", priv$.reconnect_attempt, " starting...")

        tryCatch(
          {
            # Check / refresh token before reconnecting
            token_valid <- tryCatch(rd_VerifyToken(), error = function(e) FALSE)
            if (!isTRUE(token_valid)) {
              if (debug) message("[DEBUG] Token expired, refreshing via rd_handshake()")
              rd_handshake()
            }

            # Get connection params
            access_token <- refinitiv_vault_get("access_token")
            url <- get_streaming_url(priv$.stream_type, priv$.port)
            protocol <- get_streaming_protocol(priv$.stream_type)
            headers <- create_streaming_headers(access_token)

            if (debug) message("[DEBUG] Creating new WebSocket: ", url)

            # Create new WebSocket — login is sent from onOpen handler
            priv$.ws <- websocket::WebSocket$new(
              url = url,
              protocols = protocol,
              headers = headers,
              autoConnect = FALSE
            )
            priv$.setup_websocket_handlers(debug = debug)
            priv$.ws$connect()

            # Don't poll or block — onOpen sends login, .handle_refresh
            # detects login success and replays subscriptions.
            # The reconnecting flag stays TRUE until login succeeds.
          },
          error = function(e) {
            if (debug) message("[DEBUG] Reconnect attempt failed: ", e$message)
            # Schedule next attempt (onClose may or may not fire depending on
            # where the error occurred; schedule explicitly to be safe)
            if (priv$.reconnecting && !priv$.user_initiated_close) {
              priv$.reconnecting <- FALSE
              priv$.attempt_reconnect(debug = debug)
            }
          }
        )
      }, delay = delay)
    },
    .fire_reconnect_callbacks = function(event, attempt) {
      callbacks <- private$.reconnect_callbacks[[event]]
      if (is.null(callbacks)) {
        return()
      }
      for (cb in callbacks) {
        tryCatch(cb(attempt), error = function(e) {
          warning("Error in ", event, " callback: ", e$message)
        })
      }
    },
    .setup_websocket_handlers = function(debug = FALSE) {
      # Error handler — guard against post-disconnect callbacks
      private$.ws$onError(function(event) {
        if (private$.user_initiated_close || is.null(private$.ws)) return()
      ...
      # Close handler — detect accidental vs user-initiated close
      private$.ws$onClose(function(event) {
        if (debug) message("[DEBUG] WebSocket closed (user_initiated=", private$.user_initiated_close, ")")
        private$.connection_state <- "disconnected"
        private$.logged_in <- FALSE
        # Reset reconnecting flag so a new attempt chain can start
        # (handles case where WS failure fires onClose while reconnecting)
        private$.reconnecting <- FALSE

        auto_reconnect <- getOption("refinitiv_streaming_auto_reconnect", TRUE)
        if (!private$.user_initiated_close && auto_reconnect) {
          if (debug) message("[DEBUG] Accidental close detected, scheduling reconnect")
          private$.attempt_reconnect(debug = debug)
        }
      })

      # Open handler — on reconnect, send login asynchronously (no blocking poll)
      private$.ws$onOpen(function(event) {
        if (debug) message("[DEBUG] WebSocket opened (reconnecting=", private$.reconnecting, ")")
        if (private$.reconnecting) {
          # Reconnect path: send login from onOpen to avoid blocking
          # poll_until_connected() inside later callback (deadlock risk)
          private$.connection_state <- "connected"
          if (private$.stream_type == "pricing") {
            tryCatch(
              {
                token <- refinitiv_vault_get("access_token")
                if (is.null(token) || nchar(token) == 0) {
                  handshake <- rd_handshake()
                  token <- handshake$access_token
                }
                login_request <- create_omm_login_request(access_token = token)
                if (debug) message("[DEBUG] Reconnect: sending login from onOpen")
                private$.ws$send(login_request)
              },
              error = function(e) {
                if (debug) message("[DEBUG] Error sending login on reconnect: ", e$message)
                warning("Reconnect login failed: ", e$message)
              }
            )
          }
        }
    ...
    .handle_refresh = function(message_data, debug = FALSE) {
      ...
      if (is_login_response && !private$.logged_in) {
        ...
        # If this was a reconnect, reset state and fire callback
        was_reconnecting <- private$.reconnecting
        if (was_reconnecting) {
          if (debug) message("[DEBUG] Reconnect login successful, resetting reconnect state")
          private$.reconnecting <- FALSE
          private$.reconnect_attempt <- 0L
          private$.fire_reconnect_callbacks("on_reconnected", 0L)
        }
        ...
        # (on reconnect, .streams is preserved so this replays all subscriptions)
