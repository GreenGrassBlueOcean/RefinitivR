...
#' }
CustomInstrumentUDCBuilder <- function(
  root,
  rollover,
  spreadAdjustment
) {
...
#' }
rd_ManageCustomInstruments <- function(
  ...
  if (toupper(operation) %in% c("CREATE")) {
    ...
    AlreadyExists <- RDObject$manage_custom_instrument(
      operation = "GET",
      symbol = symbol
    )
    ...
    if (!is.null(AlreadyExists$state$code)) {
      Request <- RDObject$create_custom_instrument(
        symbol = symbol,
        formula = formula,
        type = type,
        basket = basket,
    ...
    if (is.null(AlreadyExists$state$code)) {
      if (toupper(operation) == "DELETE") {
        Request <- RDObject$manage_custom_instrument(
          operation = operation,
          symbol = symbol
        )
...
#' }
rd_SearchCustomInstruments <- function(RDObject = rd_connection(), debug = TRUE) {
