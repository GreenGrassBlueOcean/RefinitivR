...
Stream <- R6::R6Class("Stream",
  public = list(
    ...
    open = function() {
      ...
      # Create message handler (shared across all stream IDs for this Stream)
      handler <- function(message_data, message_type = NULL) {
        private$.handle_message(message_data, message_type)
      }

      # Subscribe to stream — returns integer vector (one ID per instrument)
      if (debug) message("[DEBUG] Calling subscribe() for stream definition...")
      private$.stream_ids <- private$.manager$subscribe(private$.definition)
      if (debug) message("[DEBUG] Subscribe returned stream_ids: ", paste(private$.stream_ids, collapse = ", "))

      # Register handler for each stream ID
      for (sid in private$.stream_ids) {
        private$.manager$register_handler(sid, handler)
        if (debug) message("[DEBUG] Handler registered for stream_id: ", sid)
      }
    ...
    close = function() {
      ...
      # Remove handlers from manager IMMEDIATELY to stop routing messages
      for (sid in private$.stream_ids) {
        if (debug) message("[DEBUG] Removing handler for stream_id: ", sid)
        private$.manager$unregister_handler(sid)
      }

      # Send Close messages and unsubscribe for each stream ID
      universe <- private$.definition$get_universe()
      ws <- private$.manager$.__enclos_env__$private$.ws

      for (i in seq_along(private$.stream_ids)) {
        sid <- private$.stream_ids[i]
        # Determine instrument name for this stream ID
        instrument <- if (i <= length(universe)) universe[i] else universe[1]

        # Send OMM Close message
        if (!is.null(ws)) {
          ...
          if (debug) message("[DEBUG] Sending unsubscribe request: ", close_request)
          tryCatch(
            {
        ...
        private$.manager$unsubscribe(sid)
      }

      # Flush pending messages once after all close requests
      if (!is.null(ws) && requireNamespace("later", quietly = TRUE)) {
        later::run_now(0.1)
      }
    ...
    #' @description Register reconnecting callback (fired when reconnect attempt starts)
    #' @param callback Function with signature (stream, attempt_number)
    #' @return self (for chaining)
    on_reconnecting = function(callback) {
      if (!is.function(callback)) {
        stop("callback must be a function")
      }
      # Register with manager so it fires during reconnect
      private$.manager$register_reconnect_callback(
        "on_reconnecting",
        function(attempt) callback(self, attempt)
      )
      return(self)
    },

    #' @description Register reconnected callback (fired after successful reconnect)
    #' @param callback Function with signature (stream, attempt_number)
    #' @return self (for chaining)
    on_reconnected = function(callback) {
      if (!is.function(callback)) {
        stop("callback must be a function")
      }
      private$.manager$register_reconnect_callback(
    ...
    plot_live = function(field = NULL, instrument = NULL, xrange = 30, # nocov start
      ...
      server <- function(input, output, session) {
        ...
        output$live_plot <- shiny::renderPlot({
          ...
          if (is.null(history) || nrow(history) == 0) {
            plot(1,
              type = "n", xlab = "Time", ylab = field_ref,
          ...
          if (length(values) >= 3 && length(timestamps) >= 3) {
            ...
            text(last_3_times, last_3,
              labels = round(last_3, 4),
    ...
    get_summary = function(instrument = NULL) {
      ...
      if (!is.null(instrument)) {
        filter_inst <- instrument
        history <- history[history$instrument == filter_inst, ]
      }
  ...
  private = list(
    ...
    .handle_update = function(instrument, fields_data, message_data) {
      ...
      if (debug) {
        message(
          "[DEBUG] .handle_update: Latest data updated. History rows before: ",
