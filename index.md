...
**RefinitivR** is an R interface to LSEG Workspace (formerly Refinitiv
Eikon) and Refinitiv DataStream.

**Just load and go** — the package auto-detects your running terminal
and connects on the first API call. No API key, no setup code, no
connection objects to manage. Repeated requests can be served from an
opt-in **session-scoped cache** with configurable TTLs.
...
- **Real-time streaming** — WebSocket-based live market data with
  callbacks
...
- **Custom instruments** — create, update, delete basket and formula
  instruments
...
> [NEWS.md](https://greengrassblueocean.github.io/RefinitivR/NEWS.md)
> for details.

This package is in no way affiliated with Thomson Reuters, Refinitiv,
Eikon, Datastream, or LSEG. A subscription to Eikon/LSEG Workspace
and/or Datastream is required. Use at your own risk!
...
which auto-creates and caches a connection on first use.
...
The package includes a real-time streaming API for live market data
...
is the modern, robust replacement for the legacy `EikonGetSymbology`. It
resolves gaps in the LSEG native symbology API by introducing a
multi-tiered fallback strategy that successfully maps complex cases
(such as historical delisted Bare RICs) while returning a clean,
`data.table` formatted result.
...
`rd_ConvertSymbol` automatically handles “Bare RICs” by navigating
through active primary instruments and historical canonical mappings.
This is critical for backtesting workflows.
...
The legacy `Eikon*` functions now also default to
[`rd_connection()`](https://greengrassblueocean.github.io/RefinitivR/reference/rd_connection.md),
so no explicit connection is needed:
...
All data retrieval functions support opt-in session-scoped caching to
...
All output uses [`message()`](https://rdrr.io/r/base/message.html), so
[`suppressMessages()`](https://rdrr.io/r/base/message.html) works as
expected.
...
5.  **Use
    ...
    in long-running scripts to process events
