...
HolidayVectorperCountry <- lapply(
  X = sort(unique(NagerHolidayData$countryCode)),
  FUN = function(x, NagerHolidayData) {
    na.omit(as.Date(unique(unlist(NagerHolidayData[NagerHolidayData$countryCode == x, ]$date)), origin = "1970-01-01"))
  },
  NagerHolidayData
)
...
NagerHolidayData <- list(
  "NagerHolidayData" = NagerHolidayData,
  "HolidayVectorperCountry" = HolidayVectorperCountry
)

HolidayDataFrame <- lapply(
  X = NagerHolidayData$HolidayVectorperCountry,
  FUN = function(x, CountryNames) {
    data.frame(x, rep(CountryNames, length(x)))
  },
