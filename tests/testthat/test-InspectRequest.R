...
test_that("InspectRequest with error structure", {
  df <- list(
    ...
    error = list(
      list(code = 251658243L, col = 1L, message = "'The record could not be found' for the instrument 'WRONRIC'", row = 0L),
      list(code = 416L, col = 2L, message = "Unable to collect data for the field 'TR.CompanyName' and some specific identifier(s).", row = 0L)
    ),
    headerOrientation = "horizontal", headers = list(list(
...
dump_refinitiv_options("test-InspectRequest")
