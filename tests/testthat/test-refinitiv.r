...
test_that("EikonGetData handles wrong RIC gracefully", {
  ...
  result <- EikonGetData(
    EikonObject = Eikon, rics = "WRONRIC",
    Eikonformulas = c("RDN_EXCHD2", "TR.CompanyName"),
    raw_output = FALSE
  )
...
test_that("EikonGetData handles mix of good and wrong RICs", {
  ...
  result <- EikonGetData(
    EikonObject = Eikon, rics = c("WRONGRIC", "MMM"),
    Eikonformulas = c("RDN_EXCHD2", "TR.CompanyName"),
    verbose = TRUE
  )
...
test_that("EikonShowAttributes returns an error when it should", {
  expect_error(
    EikonShowAttributes(EikonObject = NULL),
...
test_that("EikonChunker satisfies basic splitting", {
  ...
  expect_equal(
    EikonChunker(
      RICS = rep("a", times = 200), MaxCallsPerChunk = 100,
...
test_that("EikonChunker respects MaxRicsperChunk = 20", {
  expect_error(
    {
      SplittedRics <- EikonChunker(
        RICS = rep("a", times = 200), Duration = 7,
...
#####
# Q5: Optimal chunk distribution
#####

test_that("EikonChunker produces minimum chunks and respects MaxRicsperChunk", {
  # 900 RICs / 300 max → exactly 3 chunks, not 4
  chunks <- EikonChunker(
    RICS = rep("a", 900), Eikonfields = "F1",
    MaxRicsperChunk = 300
  )
  expect_equal(length(chunks), 3L)
  expect_true(all(sapply(chunks, length) <= 300))
  expect_equal(sum(sapply(chunks, length)), 900)
})

test_that("EikonChunker distributes evenly across chunks (no runt chunks)", {
  # 601 RICs / 300 max → 3 chunks of ~200 each (not 299/299/3)
  chunks <- EikonChunker(
    RICS = rep("a", 601), Eikonfields = "F1",
    MaxRicsperChunk = 300
  )
  sizes <- sapply(chunks, length)
  expect_equal(length(chunks), 3L)
  expect_true(all(sizes <= 300))
  # The old code produced a runt chunk of 3. Verify the smallest chunk
  # is at least half the largest (i.e. roughly even).
  expect_gte(min(sizes), ceiling(max(sizes) / 2))
})

test_that("EikonChunker handles both constraints simultaneously", {
  # 1000 RICs, 50 fields → 50,000 dp / 12,000 = 5 chunks (dp-constrained)
  # 1000 / 300 = 4 chunks (ric-constrained)
  # Should use 5 chunks (the binding constraint)
  chunks <- EikonChunker(
    RICS = rep("a", 1000),
    Eikonfields = paste0("F", 1:50),
    MaxRicsperChunk = 300
  )
  expect_equal(length(chunks), 5L)
  expect_true(all(sapply(chunks, length) <= 300))
  expect_equal(sum(sapply(chunks, length)), 1000)
})

test_that("EikonChunker handles single RIC", {
  chunks <- EikonChunker(RICS = "AAPL.O", Eikonfields = c("F1", "F2"))
  expect_equal(length(chunks), 1L)
  expect_equal(chunks[[1]], "AAPL.O")
})
...
test_that("EikonPostProcessor satisfies testcases", {
  expect_equal(
    EikonPostProcessor(Eikon_get_dataOuput = StartTestEikonData),
...
test_that("EikonGetData can handle long requests gracefully", {
  ...
  fields <- c(
    "CURRENCY", "TR.AvgDailyVolume6M", "TR.InstrumentType",
  ...
  Rics <- c(
    "UMI.BR", "DTEGn.DE", "SAPG.DE", "ALVG.DE", "NOKIA.HE",
    "NZYMb.CO", "0RUY.L", "NVJP.F"
  )

  result <- EikonGetData(
    EikonObject = Eikon, rics = Rics,
...
restore_refinitiv_state(.saved_state, "test-refinitiv")
