...
test_that("RDPget_search_metadata caches result and returns hit on second call", {
  call_count <- 0L
  dummy_conn <- list(
    get_search_metadata = function(searchView) {
      call_count <<- call_count + 1L
      data.frame(Field = "BID", Type = "Float")
    }
  )
  ...
  result1 <- RDPget_search_metadata(RDP = dummy_conn, searchView = "EquityQuotes", cache = TRUE)
  result2 <- RDPget_search_metadata(RDP = dummy_conn, searchView = "EquityQuotes", cache = TRUE)

  # Connection called only once — second call served from cache
...
restore_refinitiv_state(.saved_state, "test-RDPget_search_metadata")
