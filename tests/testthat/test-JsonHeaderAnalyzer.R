test_that("JsonHeaderAnalyzer works", {
  jsonreturn <- list(list(
    links = list(count = 8L), variability = "", universe = list(
      list(
        Instrument = "GOOG.O", `Company Common Name` = "Alphabet Inc",
        `Organization PermID` = "5030853586", `Reporting Currency` = "USD"
      ),
      list(
        Instrument = "MSFT.O", `Company Common Name` = "Microsoft Corp",
        `Organization PermID` = "4295907168", `Reporting Currency` = "USD"
      )
    ),
    data = list(
      list("GOOG.O", "2022-12-31T00:00:00", "2022-12-31T00:00:00", 264281.9584, 146357.8752),
    ...
    headers = list(
      list(name = "instrument", title = "Instrument", type = "string", description = "The requested Instrument as defined by the user."),
      list(
      ...
      list(
        name = "TR.Revenue", title = "Revenue", type = "number",
...
test_that("JsonHeaderAnalyzer works for historical pricing", {
  test <- list(
    universe = list(ric = "VOD.L"), status = list(
  ...
  CorrectOutcome <- c(
    "DATE", "TRDPRC_1", "MKT_HIGH", "MKT_LOW",
...
test_that("JsonHeaderAnalyzer works for historical pricing", {
  inputjson <- list(
    links = list(count = 8L), variability = "",
    universe = list(
...
test_that("rd_OutputProcesser can work with complete NA responses ", {
  inputjson <- list(
    ...
    error = list(
      list(code = 218L, col = 1L, message = "The formula must contain at least one field or function.", row = 0L),
  ...
  suppressMessages(expect_equal(
    JsonHeaderAnalyzer(inputjson, Selectedheader = "displayName", use_field_names_in_headers = TRUE),
