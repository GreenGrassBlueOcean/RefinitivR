...
test_that("rd_GetData satisfies basic request", {
  ...
  Eikonformulas <- c(
    "TR.PriceClose.Currency", "TR.InstrumentType", "TR.ExchangeName",
    "TR.ExchangeMarketIdCode", "TR.InstrumentIsActive", "TR.ISINCode"
  )

  result <- rd_GetData(
    rics = RICS, Eikonformulas = Eikonformulas,
    RD = RDConnect()
  )

  expect_s3_class(result, "data.frame")
  expect_equal(sort(names(result)), sort(c(
    "Instrument", "Currency", "Instrument Type", "Exchange Name",
...
test_that("rd_GetData can handle previous problem case", {
  ...
  corp_event_rd <- rd_GetData(
    SpaceConvertor = ".", RD = RDConnect(), ,
    rics = asset,
...
restore_refinitiv_state(.saved_state, "test-rd_GetData")
