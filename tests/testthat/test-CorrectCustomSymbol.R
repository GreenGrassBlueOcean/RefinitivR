test_that("CorrectCustomInstrument works", {
  ...
  expect_equal(
    CorrectCustomInstrument(symbol = "S)test.ABCDE-123456", UUID = "ABCDE-123456"),
    "S)test.ABCDE-123456"
  )
  expect_equal(
...
test_that("CorrectCustomInstrument fails when it should", {
  expect_error(CorrectCustomInstrument(symbol = "testABCDE-123456", UUID = "ABCDE-123456"),
    "Custom Symbol 'testABCDE-123456' can not be corrected by CorrectCustomInstrument Check manually",
    fixed = TRUE
  )
})
...
test_that("CheckifCustomInstrument works", {
  ...
  expect_equal(
    CheckifCustomInstrument(symbol = c("S)test.ABCDE-123456", "AAPL.O"), UUID = "ABCDE-123456"),
    c(TRUE, FALSE)
  )

  expect_equal(
    CheckifCustomInstrument(symbol = c("NVDA.O", "AAPL.O"), UUID = "ABCDE-123456"),
    c(FALSE, FALSE)
  )
})
...
test_that("CustomInstrumentHolidayBuilder works", {
  expect_equal(
    ...
    structure(
      list(
        list(date = "2023-12-01", reason = "Special Bank Holiday 1"),
...
dump_refinitiv_options("test-CorrectCustomSymbol")
