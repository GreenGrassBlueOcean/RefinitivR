...
# NOTE: Use setup_mock_json_env() from helper.R — it sets the vault correctly
# and uses refinitiv_base_url = "http://lh" to match lh-9000/ fixture paths.
...
test_that("EikonGetData single RIC from fixture", {
  ...
  with_mock_dir("ekdata-1ric", {
    result <- EikonGetData(
      EikonObject = RD, rics = "MMM",
      Eikonformulas = c(
        "TR.CompanyName", "CF_EXCHNG",
        "RDN_EXCHD2", "CURRENCY"
      )
    )
  })
...
test_that("EikonGetData multi RIC from fixture", {
  ...
  with_mock_dir("ekdata-nric", {
    result <- EikonGetData(
      EikonObject = RD, rics = c("MMM", "III.L"),
      Eikonformulas = c(
        "TR.CompanyName", "CF_EXCHNG",
        "RDN_EXCHD2", "CURRENCY"
      )
...
test_that("rd_handshake from fixture", {
  ...
  withr::local_options(eikon_port = 9000L, refinitiv_base_url = "http://lh")
  # Fixture was recorded with DEFAULT_WORKSPACE_APP_KEY in the payload
...
#
# Each test populates the cache inside a mock, then verifies that a second
# call outside the mock (no HTTP backend) returns the cached result.
# =============================================================================

test_that("rd_GetData caching works (store + hit)", {
  skip_if(!fixture_ready("getdata-1ric"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()
...
test_that("rd_GetHistoricalPricing caching works (store + hit)", {
  skip_if(!fixture_ready("histpx-1d"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()
  ...
  with_mock_dir("histpx-1d", {
    result1 <- rd_GetHistoricalPricing(
      RDObject = RD, universe = "VOD.L",
      interval = "P1D", count = 20L, cache = TRUE
    )
  })
...
test_that("rd_GetHistory caching works (store + hit)", {
  skip_if(!fixture_ready("hist-px"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  with_mock_dir("hist-px", {
    result1 <- rd_GetHistory(
      RD = RD, universe = "AAPL.O",
      start = "2020-01-02", end = "2020-01-10",
      interval = "P1D", cache = TRUE
    )
  })

  result2 <- rd_GetHistory(
    RD = RD, universe = "AAPL.O",
    start = "2020-01-02", end = "2020-01-10",
    interval = "P1D", cache = TRUE
  )
...
test_that("EikonGetTimeseries caching works (store + hit)", {
  skip_if(!fixture_ready("ekts-daily"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  with_mock_dir("ekts-daily", {
    result1 <- EikonGetTimeseries(
      EikonObject = RD, rics = "MMM",
      start_date = "2020-01-01T01:00:00",
      end_date = "2020-01-10T01:00:00",
      corax = "unadjusted", cache = TRUE
    )
  })

  result2 <- EikonGetTimeseries(
    EikonObject = RD, rics = "MMM",
    start_date = "2020-01-01T01:00:00",
    end_date = "2020-01-10T01:00:00",
    corax = "unadjusted", cache = TRUE
...
test_that("EikonGetData caching works (store + hit)", {
  skip_if(!fixture_ready("ekdata-1ric"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  with_mock_dir("ekdata-1ric", {
    result1 <- EikonGetData(
      EikonObject = RD, rics = "MMM",
      Eikonformulas = c(
        "TR.CompanyName", "CF_EXCHNG",
        "RDN_EXCHD2", "CURRENCY"
...
test_that("EikonGetSymbology caching works (store + hit)", {
  skip_if(!fixture_ready("eksym-r2i"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()
...
test_that("RDPsearch caching works (store + hit)", {
  skip_if(!fixture_ready("search-basic"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  with_mock_dir("search-basic", {
    result1 <- RDPsearch(RDP = RD, query = "AAPL.O", top = 5, cache = TRUE)
  })

  result2 <- RDPsearch(RDP = RD, query = "AAPL.O", top = 5, cache = TRUE)

  expect_equal(result2, result1)
  rd_ClearCache()
})

test_that("RDPget_search_metadata caching works (store + hit)", {
  skip_if(!fixture_ready("searchmeta-eq"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  with_mock_dir("searchmeta-eq", {
    result1 <- RDPget_search_metadata(
      RDP = RD, searchView = "EquityQuotes", cache = TRUE
    )
  })

  result2 <- RDPget_search_metadata(
    RDP = RD, searchView = "EquityQuotes", cache = TRUE
  )
...
test_that("rd_get_news_headlines caching works (store + hit)", {
  skip_if(!fixture_ready("news-hl-msft"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  with_mock_dir("news-hl-msft", {
    result1 <- rd_get_news_headlines(
      RDObject = RD, query = "R:MSFT.O", limit = 2L, cache = TRUE
    )
  })

  result2 <- rd_get_news_headlines(
    RDObject = RD, query = "R:MSFT.O", limit = 2L, cache = TRUE
  )
...
test_that("rd_get_news_story caching works (store + hit)", {
  skip_if(!fixture_ready("news-story"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  # Get a valid story_id from the fixture, then test caching on the story fetch
  with_mock_dir("news-story", {
    headlines <- rd_get_news_headlines(
      RDObject = RD, query = "R:MSFT.O", limit = 1L
    )
    result1 <- rd_get_news_story(
      RDObject = RD, story_id = headlines$storyId[1], cache = TRUE
    )
  })

  result2 <- rd_get_news_story(
    RDObject = RD, story_id = headlines$storyId[1], cache = TRUE
  )
...
test_that("rd_get_top_news caching works (store + hit)", {
  skip_if(!fixture_ready("news-top"), "Fixture not available")
  RD <- setup_mock_json_env()
  rd_ClearCache()

  with_mock_dir("news-top", {
    result1 <- rd_get_top_news(RDObject = RD, cache = TRUE)
  })

  result2 <- rd_get_top_news(RDObject = RD, cache = TRUE)

  expect_equal(result2, result1)
  rd_ClearCache()
})

restore_refinitiv_state(.saved_state, "test-zzz-httptest2-integration")
