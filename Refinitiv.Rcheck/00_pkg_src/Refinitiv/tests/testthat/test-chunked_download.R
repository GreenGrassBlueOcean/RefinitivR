...
test_that("chunked_download retries failed chunks", {
  ...
  results <- chunked_download(
    ...
    fetch_fn = function(j) {
      ...
      if (j == 2 && call_counts[j] <= 2) return(NA)
      j * 10
    },
    sleep   = 0,
    backoff = 0
...
test_that("chunked_download respects custom is_success predicate", {
  ...
  expect_error(
    chunked_download(
      n_chunks   = 1,
      fetch_fn   = function(j) { call_count <<- call_count + 1L; list() },
...
test_that("chunked_download default max_retries is 3", {
  ...
  expect_error(
    chunked_download(
      n_chunks = 1,
      fetch_fn = function(j) { call_count <<- call_count + 1L; NA },
      sleep    = 0,
      backoff  = 0
    )
  )
...
test_that("chunked_download backoff only fires for failed rounds", {
  ...
  results <- chunked_download(
    ...
    fetch_fn = function(j) {
      call_counts[j] <<- call_counts[j] + 1L
      if (j == 2 && call_counts[j] == 1) return(NA)
      j * 10
    },
    sleep   = 0,
    backoff = 1.0
