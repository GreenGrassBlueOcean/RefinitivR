...
#' }
rd_GetHistory <- function(RD = rd_connection()
                         , universe = NULL
                         , fields = NULL
                         , parameters = NULL
                         , interval = NULL
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("rd_GetHistory", universe, fields, parameters,
...
.check_date_granularity <- function(hp_dt, gd_dt) {
  Instrument <- hp_n <- gd_n <- ratio <- NULL
  hp_counts <- hp_dt[, .(hp_n = data.table::uniqueN(Date)), by = Instrument]
  gd_counts <- gd_dt[, .(gd_n = data.table::uniqueN(Date)), by = Instrument]
  # Inner join: only instruments present on both sides

  merged <- data.table::merge.data.table(hp_counts, gd_counts, by = "Instrument")
  if (nrow(merged) == 0L) return(invisible(NULL))
