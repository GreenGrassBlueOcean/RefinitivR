...
#'
#' }
rd_GetHistoricalPricing <- function( RDObject = rd_connection()
                                   , universe = NULL
                                   , interval = "P1D"
                                   , start = NULL
                                   , end = NULL
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("rd_GetHistoricalPricing", universe, interval, start, end,
...
rd_OutputProcesser <- function(x, use_field_names_in_headers = TRUE, SpaceConvertor = NULL){
  ...
  if(!identical(OtherDateColumns, integer(0) )){
    ...
    if(identical(DateType, "numeric")){
      ...
      return_DT[, (OtherDateColumns) := lapply(.SD, as.Date), .SDcols = OtherDateColumns]
    }
  }
