...
PropertiesActiveRefinitivObject <- function(verbose = TRUE){
  ...
  if(verbose){
    message(paste("Refinitiv Connection method =", ModuleName, "\n",
                  "Type =", Type))
  }
...
retry <- function(fn, max_attempts = getOption("refinitiv_max_retries", 2L),
  ...
  for (i in seq_len(max_attempts)) {
    result <- tryCatch(fn(), error = function(e) {
      last_error <<- e
      message("API request failed (attempt ", i, "/", max_attempts, "): ",
              conditionMessage(e))
      if (i < max_attempts) {
    ...
    if (!inherits(result, "retry_error_sentinel")) return(result)
  }
...
#'
#'
EikonGetTimeseries <- function(EikonObject = rd_connection(), rics, interval = "daily", calender = "tradingdays", corax = "adjusted", fields = c('TIMESTAMP', 'VOLUME', 'HIGH', 'LOW', 'OPEN', 'CLOSE')
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("EikonGetTimeseries", rics, interval, calender, corax,
  ...
  TimeSeriesList <- chunked_download(
    ...
    fetch_fn     = function(j) {
      ...
      result <- if (is.null(fields) || length(fields) == 0L) {
        retry(function() EikonObject$get_timeseries(
          rics       = ChunckedRics[[j]],
          interval   = interval,
          calendar   = calender,
          fields     = c(),
...
#'
#'
EikonGetData <- function(EikonObject = rd_connection(), rics, Eikonformulas, Parameters = NULL, raw_output = FALSE, time_out = 60, verbose = FALSE, SpaceConvertor = "."
                        , cache = NULL){

  # ── Cache lookup ──
  ttl <- resolve_cache(cache, fn_default_ttl = 300)
  if (!isFALSE(ttl)) {
    .ck <- cache_key("EikonGetData", rics, Eikonformulas, Parameters,
...
EikonDataList <- chunked_download(
  ...
  fetch_fn     = function(j) {
    ...
    result <- retry(function() EikonObject$get_data(
...
#'
#'
EikonGetSymbology <- function( EikonObject = rd_connection(), symbol, from_symbol_type = "RIC", to_symbol_type = c('CUSIP', 'ISIN', 'SEDOL', 'RIC', 'ticker', 'lipperID', 'IMO', 'OAPermID')
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("EikonGetSymbology", symbol, from_symbol_type, to_symbol_type,
  ...
  EikonSymbologyList <- chunked_download(
    ...
    fetch_fn     = function(j) {
      ...
      result <- retry(function() EikonObject$get_symbology(
