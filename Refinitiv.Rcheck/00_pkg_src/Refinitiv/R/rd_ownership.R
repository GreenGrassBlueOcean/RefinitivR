...
#'   rd_GetOwnership(universe = "AAPL.O",
#'                   view = "consolidated/breakdown",
#'                   stat_type = 1)
#'
#'   # Top fund holders (first 50)
#'   rd_GetOwnership(universe = "MSFT.O",
#'                   view = "fund/investors",
#'                   limit = 50)
#'
#'   # Insider transactions
#'   rd_GetOwnership(universe = "TSLA.O",
...
rd_GetOwnership <- function(RDObject = rd_connection(),
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("rd_GetOwnership", universe, view, stat_type, limit,
  ...
  if (needs_pagination || fetch_all) {
    ...
    if (raw_output) {
      ...
      ReturnElement <- data.table::rbindlist(
        pages_df, use.names = TRUE, fill = TRUE
      ) |> data.table::setDF()
    }
