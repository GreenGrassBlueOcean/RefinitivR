...
RDPShowAvailableSearchViews <- function(){
  c("BondFutOptQuotes", "CdsInstruments", "CdsQuotes", "CmoInstruments",
...
#' }
GetSearchView <- function(ConnectionObject = rd_connection()
                        , SearchView = NULL){
...
#'
#' }
RDPsearch <- function(RDP = rd_connection()
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("RDPsearch", query, view, select, top, filter, boost,
...
#'
#' @param IPAoutput output from an RDP IPA FinancialContracts API call
#'
#' @return data.frame
#' @keywords internal
#'
