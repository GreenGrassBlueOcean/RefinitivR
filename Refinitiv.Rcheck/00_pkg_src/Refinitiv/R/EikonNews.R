...
#' }
EikonGetNewsHeadlines <- function(EikonObject = rd_connection()
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("EikonGetNewsHeadlines", query, count, repository,
  ...
  RawHeadlinesList <- chunked_download(
    ...
    fetch_fn     = function(j) {
      retry(function() EikonObject$get_news_headlines(
        query      = query[j],
...
#' }
EikonGetNewsStory <- function(EikonObject = rd_connection()
  ...
  EikonNewsList <- chunked_download(
    ...
    fetch_fn     = function(j) {
      retry(function() EikonObject$get_news_story(
        story_id   = story_id[j],
        debug      = debug,
