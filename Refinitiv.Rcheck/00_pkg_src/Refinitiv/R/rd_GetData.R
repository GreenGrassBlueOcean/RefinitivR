...
#'  test_json <- rd_GetData( RDObject = Refinitiv
#'                         , rics =  rics
...
#'                         )
#'}
rd_GetData <- function(RDObject = rd_connection(), rics, Eikonformulas, Parameters = NULL, raw_output = FALSE
  ...
  if (!isFALSE(ttl)) {
    .ck <- cache_key("rd_GetData", rics, Eikonformulas, Parameters,
  ...
  EikonDataList <- chunked_download(
    ...
    fetch_fn     = function(j) {
      retry(function() RDObject$get_data(
